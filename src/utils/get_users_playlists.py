...
def __fetch_users_playlists(sp: SpotifyOAuth) -> None:
    ...
    return {"playlists": user_playlists, "playlists_count": user_playlists_counter}


def __print_check_details(playlists: dict, check_time: float) -> None:
    """Get info from check and print summary of it.

    Args:
        playlists (dict): Playlists, owned by user
        check_time (float): Elapsed time on checking
    """
    print("\n\n======== Check summary ========")
    if playlists["playlists_count"] == 0:
        print("There are no playlists owned by you")
        print(f"All playlists checked in {check_time} seconds!")
        print("===============================")
        return
    print(
        f"Found {playlists['playlists_count']} playlist(s) owned by you in {check_time} seconds!" \
